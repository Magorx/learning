...
int32_t IntersectionSize(std::vector<Point> points, int32_t balance_lvl) {
    ...
    for (size_t i = 1; i < points.size(); ++i) {
        ...
        if (balance > 0 && point.type == END) {
            return point.coord - points[i - 1].coord + 1;
        }
    }
    return 0;
}
...
int main() {
    ...
    int32_t begin = std::max(segments[0].begin.coord, segments[1].begin.coord);
    int32_t end = std::min(segments[0].end.coord, segments[1].end.coord);
    std::cout << std::max(end - begin + 1, 0) << '\n';
