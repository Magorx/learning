...
int32_t PeresechenieSize(std::vector<Point> points) {
    int32_t balance = 0;
    int32_t min_balance = 1e9;
...
int main() {
    ...
    for (int32_t i = 0; i < points_count; ++i) {
        ...
        for (int32_t j = 0; j < n; ++j) {
            int32_t h = 0, m = 0, s = 0;
            int32_t sum = 0;
            //scanf("%d:%d:%d %d", &h, &m, &s, &sum);
            char trash;
            std::cin >> h >> trash >> m >> trash >> s >> sum;
            int32_t time = h * 60 * 60 + m * 60 + s;
            points.push_back(Point{time, sum});
        }
    }
    std::sort(points.begin(), points.end());

    std::cout << std::abs(std::min(PeresechenieSize(points), 0)) << '\n';
