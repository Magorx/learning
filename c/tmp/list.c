...
struct list_node *list_node_construct(struct user_data *data) {
	...
	new_node = (struct list_node*)many_attempts_calloc(
		1, sizeof(struct list_node), MAX_MEMORY_ALLOCATION_ATTEMPTS);
    if (new_node == NULL) {
        return NULL;
    }
...
struct list *list_construct() {
	struct list *new_list = (struct list*)many_attempts_calloc(
		1, sizeof(struct list), MAX_MEMORY_ALLOCATION_ATTEMPTS);
	if (new_list == NULL) {
		return NULL;
	}
...
int list_push_back(struct list *self, struct user_data *new_data) {
